...
void Read_matrix(int local_mat[], int n, int my_rank, int p, MPI_Comm comm)
{
    ...
    if (my_rank == 0)
    {
        ...
        MPI_Scatter(temp_mat, n*n/p, MPI_INT, local_mat, n*n/p, MPI_INT, 0, comm); free(temp_mat);
    }
    else
    {
...
void Print_row(int local_mat[], int n, int my_rank, int i)
{
    char char_int[100]; char char_row[1000]; int j, offset = 0;
    for (j = 0; j < n; j++)
    {
        if (local_mat[i*n + j] == INFINITY) 
            sprintf(char_int, "i ");
    else
        sprintf(char_int, "%d ", local_mat[i*n + j]); sprintf(char_row + offset, "%s", char_int); offset += strlen(char_int);
    }
    ...
    for (global_k = 0; global_k < n; global_k++)
    {
        ...
        for (local_i = 0; local_i < n/p; local_i++)
            for (global_j = 0; global_j < n; global_j++)
            {
                ...
                if (temp < local_mat[local_i*n+global_j]) local_mat[local_i*n + global_j] = temp;
            }
    }
    free(row_k);
}
